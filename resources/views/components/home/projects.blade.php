<section id="projects" class="flex flex-col justify-center w-full px-5 mb-10 gap-10">
    ...
    <div class="flex flex-col gap-10 items-center lg:grid lg:grid-cols-3">
        <x-home.cardComponent x-data="{ show: false }" x-intersect.once="show = true" x-data="{ show: false }" x-intersect.once="show = true"
        x-bind:class="{ 'is-visible': show }" 
            class="fade-card" title="Cesar's Coffee Cup"
            description="Coffee business website with wholesale acces." role="Full Build" stack="Laravel / PHP / MySQL"
            deployment="Docker / Caddy / Ubuntu" hosting="Self-Hosted" href="https://cesarscoffeecup.com"
            img="cesarscoffeecup.png" :items="[
                'Customer Accounts',
                'Authentication',
                'Rate Limiter',
                'Contact Forms',
        ...
        <x-home.cardComponent x-data="{ show: false }" x-intersect.once="show = true" x-data="{ show: false }" x-intersect.once="show = true"
        x-bind:class="{ 'is-visible': show }" 
            class="fade-card"  title="Latina Miles Away"
            ...
            href="https://latinamilesaway.com" img="latinamilesaway.png" :items="[
                'Booking System',
                'Stripe Payment',
                'PDF Viewer',
                'PDF Downloads',
                'Responsive Design',
                'Language Translation ES|EN',
            ]"></x-home.cardComponent>
        <x-home.cardComponent x-data="{ show: false }" x-intersect.once="show = true" x-data="{ show: false }" x-intersect.once="show = true"
        x-bind:class="{ 'is-visible': show }" 
            class="fade-card"  title="Employees DB" role="Full Build"
